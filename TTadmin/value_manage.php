...
$image011=$rand1.$_FILES["image3"]["name"];
...
{
$image011=$_REQUEST['hiddenimage3'];
}
if(isset($_REQUEST['update']))
{


$query="update value_offered SET title='$title',seo_keyword='$sekey2',content='$content', date=now(),sortorder='$sortorder',image='$images_name',category='$category',image2='$image01',image3='$image011',feature='$feature',url='$url',skill='$skill',fullname='$fullname' WHERE id=$id";
...
{
$query="insert into value_offered(title,seo_keyword,content,date,sortorder,status,image,category,image2,feature,url,skill,image3,fullname) values('$title','$sekey2','$content',now(),'$sortorder','1','$images_name','$category','$image01','$feature','$url','$skill','$image011','$fullname')";
mysqli_query($link,$query);
...
$query="update value_offered SET status='$status' WHERE id='$id'";         
mysqli_query($link,$query);
}
else if(isset($_REQUEST['del']))
{
$id=$_REQUEST['del'];
$query="delete from value_offered WHERE id=$id";
mysqli_query($link,$query);
}
?>
...
$sql_cms="select * from value_offered order by sortorder ASC"; 
...
?>
                    <tr>
                      ...
                      <td><a href="value_form.php?cms=<?php echo $row_cms['id'];?>" class="edit_btn"><span>Edit</span></a>
                        <?php if($row_cms['status']==1){ ?>
                        <span><a href="value_manage.php?id=<?php echo $row_cms['id']; ?>" title="Active" class="active"><span>ac</span></a></span>
                        <?php } else { ?>
                        <span><a href="value_manage.php?id1=<?php echo $row_cms['id']; ?>" title="Inactive" class="inact"><span>in</span></a></span>
                        <?php } ?>
                        <span><a href="value_manage.php?del=<?php echo $row_cms['id']; ?>" onClick="return confirm('Do you really want to remove it?')" class="delet"><span>Delete</span></a></span></td>
                    </tr>
